...
parser = argparse.ArgumentParser(description='Train or test neutral net motor controller')
...
class PolicyNetwork(nn.Module):
    ...
    def evaluate(self, state, epsilon=1e-6):
        ...
        normal = Normal(0, 1)
        z = normal.sample()
        action_0 = torch.tanh(mean + std * z.to(device))#TanNormal distribution as actions; reparameterization trick
...
class SAC_Trainer():
    def __init__(self, replay_buffer, hidden_dim, action_range):
        ...
        self.soft_q_net3 = SoftQNetwork(state_dim, action_dim, hidden_dim).to(device)

        self.target_soft_q_net1 = SoftQNetwork(state_dim, action_dim, hidden_dim).to(device)
        self.target_soft_q_net2 = SoftQNetwork(state_dim, action_dim, hidden_dim).to(device)
        self.target_soft_q_net3 = SoftQNetwork(state_dim, action_dim, hidden_dim).to(device)
        ...
        self.soft_q_optimizer3 = optim.Adam(self.soft_q_net3.parameters(), lr=soft_q_lr)
    ...
    def update(self, batch_size, reward_scale=10, auto_entropy=True, use_demons=False, target_entropy=-2, gamma=0.99, soft_tau=1e-2):
        ...
        reward = torch.FloatTensor(reward).unsqueeze(1).to(device)# reward is a single value, unsqueeze() to add one dim to be [reward] at the sample dim;
    ...
    # Training Q Function
        target_q_min = torch.min(self.target_soft_q_net1(next_state, new_next_action), torch.min(self.target_soft_q_net2(next_state, new_next_action), self.target_soft_q_net3(next_state, new_next_action))) - self.alpha * next_log_prob
    ...
    # Training Policy Function
        predicted_new_q_value = torch.min(self.soft_q_net1(state, new_action),torch.min(self.soft_q_net2(state, new_action), self.soft_q_net3(state, new_action)))
    ...
    def load_model(self, path):
        self.soft_q_net1.load_state_dict(torch.load(path + '_q1', map_location={'cuda:0':'cpu'}))
        self.soft_q_net2.load_state_dict(torch.load(path + '_q2', map_location={'cuda:0':'cpu'}))
        self.soft_q_net3.load_state_dict(torch.load(path + '_q3', map_location={'cuda:0':'cpu'}))
        self.policy_net.load_state_dict(torch.load(path + '_policy', map_location={'cuda:0':'cpu'}))
...
def worker(id, sac_trainer, rewards_queue, replay_buffer, max_episodes, max_steps, batch_size, explore_steps,\
    ...
    env = GraspEnv(headless=headless)#no need to configure different port_nummer for calling different Vrep env at the same time
    ...
    if args.test:
        ...
        for eps in range(5):
            ...
            for step in range(max_steps):
                ...
                episode_reward += reward
                state = next_state
                #print(env.suction.check_collision(env.target))
                print(env.proximity_sensor.is_detected(env.target))
            print(env.proximity_sensor.is_detected(env.target))
            print(env.agent_ee_tip.get_position())
            #print(env.agent.get_joint_positions())
            #desired_orientation_tip = [-np.pi,0,0.001567]
            #print(np.sqrt((np.sum(np.abs(np.array(env.agent_ee_tip.get_orientation())-np.array(desired_orientation_tip))))))
            print(env.agent_ee_tip.get_orientation())
            print(env._is_holding())
            desired_orientation_tip = [-np.pi,0,0.001567]
            desired_position_tip = [0,1.5513,1.74]
            tip_x,tip_y,tip_z = env.agent_ee_tip.get_position()
            tip_row,tip_pitch,tip_yaw = env.agent_ee_tip.get_orientation()
            sqr_distance = (tip_x-desired_position_tip[0])**2 + (tip_y-desired_position_tip[1])**2 + (tip_z-desired_position_tip[2])**2 
            sqr_orientation=(tip_row-desired_orientation_tip[0])**2 + (tip_pitch-desired_orientation_tip[1])**2+(tip_yaw-desired_orientation_tip[2])**2
            print("sqr_distance:",sqr_distance)
            print("sqr_orientation:",sqr_orientation)
            print('Episode: ', eps, '| Episode Reward: ', episode_reward)
        env.shutdown()
